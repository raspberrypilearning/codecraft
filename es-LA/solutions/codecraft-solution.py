...
def recoger():
  ...
  if inventario[casillaActual] < MAXCASILLAS:
    ...
    #el jugador ahora está parado sobre tierra
...
def dibujarRecurso(y, x):
  ...
  if pintar == False:
    ...
    if jugadorX == y and jugadorY == x:
      renderizadorT.shape(jugadorImg)
      renderizadorT.stamp()
    pantalla.update()
...
def dibujarInventario():
  ...
  if pintar == False:
    ...
    for i in range(2):
      renderizadorT.forward(altura_inventario - 60)
...
pantalla.register_shape(jugadorImg)
